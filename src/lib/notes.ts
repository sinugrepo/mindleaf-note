...
async function expandIfCollapsed(parentId: string): Promise<void> {
  ...
  if (parent && !parent.isExpanded) {
    await db.notes.update(parentId, { isExpanded: true });
  }
}
...
/**
 * Soft-delete a note and all its descendants by stamping `deletedAt` on
 * each row. Recursive — matching the recursive hard-delete behavior the
 * TreeView previously used — so a folder delete takes its children with
 * it (recoverable as one subtree later).
 *
 * Idempotent: re-running on an already-deleted note refreshes the timer
 * (`deletedAt` is set to the new `now`), which is the right UX for a
 * "Move to Trash" repeat.
 *
 * Returns the list of ids that were updated — useful for tests.
 */
export async function softDeleteNote(
  noteId: string,
  now: number = Date.now(),
): Promise<string[]> {
  // collectDescendants walks parentId edges; works fine on a soft-deleted
  // subtree too (we don't care if descendants were already deleted —
  // re-stamping is idempotent).
  const allNotes = await db.notes.toArray();
  const descendants = collectDescendants(allNotes, noteId);
  const targets = [...descendants, noteId];
  await db.notes.bulkUpdate(
    targets.map((id) => ({ key: id, changes: { deletedAt: now } })),
  );
  return targets;
}

/**
 * Restore a soft-deleted note (and its descendants) by clearing
 * `deletedAt`. Recursive so a folder restored from trash brings back its
 * whole subtree in one click.
 *
 * Idempotent on already-active notes (no-op).
 *
...
export async function restoreNote(noteId: string): Promise<string[]> {
  ...
  await db.notes.bulkUpdate(
    trashedIds.map((id) => ({ key: id, changes: { deletedAt: null } })),
  );
  return trashedIds;
}
 ...
 * start (see main.tsx, fire-and-forget).
 *
...
/**
 * Permanently delete a single trash item and its descendants. Used by
 * the TrashView's "Delete forever" affordance — distinct from
 * `purgeOldTrash` because the user explicitly asked NOW. Cascade-deletes
 * attachments belonging to the purged subtree (same rationale as
 * `purgeOldTrash`).
 *
 * Refuses to operate on an ACTIVE (non-trashed) note — this is the
 * defensive last-line guard against accidentally wiping live work via a
 * future caller that wires the wrong button. The TrashView only ever
 * surfaces trashed rows, so this won't fire in the current UI; if it
 * throws, it's by design.
 *
...
export async function permanentlyDeleteNote(noteId: string): Promise<number> {
  ...
  if (isActiveNote(target)) {
    throw new Error(
      `permanentlyDeleteNote: note "${noteId}" is not in Trash — refusing to delete active note. Use softDeleteNote first or move the note through the Trash flow.`,
    );
  }
  const allNotes = await db.notes.toArray();
  const descendants = collectDescendants(allNotes, noteId);
  const targetIds = [noteId, ...descendants];
  await cascadingDeleteNotes(targetIds);
  return targetIds.length;
}

/**
 * Empty the trash: hard-delete every soft-deleted note (and their
 * attachment rows). Distinct from `purgeOldTrash` in that there is no
 * age gate — the user clicked the button and wants it GONE. Used by
 * the TrashView's confirm modal.
 *
 * Returns the number of notes hard-deleted.
 */
export async function emptyTrash(): Promise<number> {
  ...
  await cascadingDeleteNotes(trashedNotes.map((n) => n.id));
  return trashedNotes.length;
}

/**
 * Cascading-delete helper: hard-delete `noteIds` from the notes table
 * AND every attachment whose `noteId` belongs to that set. Used by all
 * three hard-delete paths (purgeOldTrash / permanentlyDeleteNote /
 * emptyTrash) so the cascade invariant is DRY-ed up in one place.
 *
