...
/**
 * Convert `note.content` HTML to a scrubbed plain-text representation
 * ready for `findActiveWikiQuery`. We strip the wikiLink span wrappers
 * to mirror what TipTap's `editor.state.doc.textContent` would
 * produce (text inside `[[…]]` is the link label, but the autocomplete
 * regex over the HTML is fine for our use-case since `[[` and `]]`
 * aren't stripped).
 *
 * Provided here so unit tests can verify the helper in isolation; the
 * Editor component passes `editor.state.doc.textContent` directly.
 */
export function htmlToPlainText(html: string): string {
  if (!html) return '';
  // Strip tags but keep their inner text. Cheap regex; we don't need
  // an HTML AST here.
