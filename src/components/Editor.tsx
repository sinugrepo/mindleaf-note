...
import { ATTACHMENT_SRC_PREFIX, Note } from '../types';
...
export function Editor({ noteId }: { noteId: string }) {
            ...
            if (file) {
              ...
              compressImage(file)
                .then(async (blob) => {
                  ...
                  if (!imageType) return;
                  // Use the schema-declared `width` field (see ResizableImage
                  // addAttributes) rather than `style` directly. TipTap's
                  // schema only honours the declared fields; matching them
                  // here keeps `node.attrs` honest when consumers later
                  // read it.
                  ...
                  // onUpdate fires synchronously during dispatch and calls
                  // saveNote, so no explicit force-save is needed here.
                })
                .catch(() => {
  ...
  // Flush unsaved content when the Editor unmounts (e.g. when the user
  // switches to another note). This catches the race where the user
  // pastes/drops an image and immediately switches: the async
  // compressImage promise may resolve after the editor is destroyed, so
  // the onUpdate -> saveNote chain never fires. latestContentRef is
  // updated on every onUpdate, so it holds the most recent editor HTML
  // even after the editor instance is gone.
  useEffect(() => {
    return () => {
