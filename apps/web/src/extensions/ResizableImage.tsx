...
async function resolveImageSrc(
  ...
  try {
    ...
    // If the blob is empty (placeholder from sync pull — metadata only,
    // blob not yet downloaded), try fetching from the backend via
    // presigned GET URL. This handles the case where a note was synced
    // from the server but the image blob hasn't been downloaded to this
    // device yet.
    if (att.blob.size === 0 && att.r2Key) {
      try {
        const attUrl = await api.getAttachmentUrl(id);
        const blobRes = await fetch(attUrl.url);
        if (blobRes.ok) {
          const blob = await blobRes.blob();
          // Cache the downloaded blob in IndexedDB so subsequent
          // renders don't re-fetch from R2.
        ...
        // R2 fetch failed — fall through to return null (placeholder).
        return null;
      }
    }

    if (att.blob.size === 0) return null;
