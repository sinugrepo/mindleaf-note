...
describe('createRootNote (db-backed)', () => {
  ...
  it('returns the same shape that buildNewNote produces, plus sync fields', async () => {
    const persisted = await createRootNote();
    const freshShape = buildNewNote({ parentId: null });
    // queuedCreateNote adds `version` and `dirty` fields for the sync
    // layer. The base shape from buildNewNote is still a subset.
    const baseKeys = Object.keys(freshShape);
    for (const key of baseKeys) {
      expect(persisted).toHaveProperty(key);
    }
    expect(persisted).toHaveProperty('version');
    expect(persisted).toHaveProperty('dirty');
  });
...
describe('renameNote (db-backed)', () => {
  ...
  it('refreshes updatedAt on rename', async () => {
    ...
    // deterministic. renameNote passes `now` through to queuedPatchNote
    // which sets it as the updatedAt value.
    const note = await createRootNote('Original');
...
describe('trash round-trip (export -> import integration)', () => {
  ...
  it('parseImportedNotes drops deletedAt so imported backups always restore as active', async () => {
    const { parseImportedNotes } = await import('../lib/notes-io');
