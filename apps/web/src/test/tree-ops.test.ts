...
describe('buildMoveSupportMap (TreeView perf path)', () => {
  it('returns empty Map for empty input', () => {
    expect(buildMoveSupportMap([]).size).toBe(0);
  });

  it('a single root row has neither move available', () => {
    const only = makeNote({ id: 'solo', order: 1 });
    const m = buildMoveSupportMap([only]);
    expect(m.size).toBe(1);
    const sup = m.get('solo')!;
    expect(sup.canMoveUp).toBe(false);
    expect(sup.otherUp).toBeUndefined();
    expect(sup.canMoveDown).toBe(false);
    expect(sup.otherDown).toBeUndefined();
  });

  it('edge sibling rows have only one move available', () => {
    // Three roots: a, b, c by order ascending.
    const a = makeNote({ id: 'a', order: 1 });
    const b = makeNote({ id: 'b', order: 2 });
    const c = makeNote({ id: 'c', order: 3 });
    const m = buildMoveSupportMap([a, b, c]);

    // 'a' is first: canMoveDown=true, canMoveUp=false
    const supA = m.get('a')!;
    expect(supA.canMoveUp).toBe(false);
    expect(supA.otherUp).toBeUndefined();
    expect(supA.canMoveDown).toBe(true);
    expect(supA.otherDown).toBe(b);

    // 'c' is last: canMoveUp=true, canMoveDown=false
    const supC = m.get('c')!;
    expect(supC.canMoveUp).toBe(true);
    expect(supC.otherUp).toBe(b);
    expect(supC.canMoveDown).toBe(false);
    expect(supC.otherDown).toBeUndefined();
  });

  it('middle row has both moves available and points at the neighbors', () => {
    const a = makeNote({ id: 'a', order: 1 });
    const b = makeNote({ id: 'b', order: 2 });
    const c = makeNote({ id: 'c', order: 3 });
    const m = buildMoveSupportMap([a, b, c]);
    const supB = m.get('b')!;
    expect(supB.canMoveUp).toBe(true);
    expect(supB.otherUp).toBe(a);
    expect(supB.canMoveDown).toBe(true);
    expect(supB.otherDown).toBe(c);
  });

  it('siblings are scoped to parentId — different folders have independent support', () => {
    // x and y are roots; a/b are children of x; c/d are children of y.
    const x = makeNote({ id: 'x', parentId: null, order: 1 });
    const y = makeNote({ id: 'y', parentId: null, order: 2 });
    const a = makeNote({ id: 'a', parentId: 'x', order: 5 });
    const b = makeNote({ id: 'b', parentId: 'x', order: 6 });
    const c = makeNote({ id: 'c', parentId: 'y', order: 7 });
    const d = makeNote({ id: 'd', parentId: 'y', order: 8 });
    const m = buildMoveSupportMap([x, y, a, b, c, d]);

    // 'b' can swap up with 'a' (same parent 'x'); its down neighbor is x's no-op
    // boundary — there is no sibling after 'b' under 'x' so canMoveDown=false.
    const supB = m.get('b')!;
    expect(supB.canMoveUp).toBe(true);
    expect(supB.otherUp).toBe(a);
    expect(supB.canMoveDown).toBe(false);
    expect(supB.otherDown).toBeUndefined();

    // 'd' can swap up with 'c', but never with 'b' (different parent).
    const supD = m.get('d')!;
    expect(supD.canMoveUp).toBe(true);
    expect(supD.otherUp).toBe(c);
    expect(supD.canMoveDown).toBe(false);
  });

  it('input order does not matter — only parentId + order do', () => {
    const a = makeNote({ id: 'a', order: 1 });
    const b = makeNote({ id: 'b', order: 2 });
    const c = makeNote({ id: 'c', order: 3 });
    const m1 = buildMoveSupportMap([a, b, c]);
    const m2 = buildMoveSupportMap([c, b, a]);
    expect(m1.get('b')!.otherUp?.id).toBe('a');
    expect(m2.get('b')!.otherUp?.id).toBe('a');
  });

  it('a note that is not in the input set has no map entry (callers treat as no-move)', () => {
    const a = makeNote({ id: 'a', order: 1 });
    const b = makeNote({ id: 'b', order: 2 });
    const m = buildMoveSupportMap([a, b]);
    expect(m.has('ghost')).toBe(false);
  });

  it('matches a hand-derived findMoveSibling on the same input (cross-check)', () => {
    // Cross-check: every (noteId, direction) entry in the map must agree
    // with a fresh findMoveSibling call on the same `notes` array. This
    // pins behavior consistency between the optimized batch path and the
    // original one-call-at-a-time helper that the unit tests already
    // exhaustively cover.
    const notes = [
      makeNote({ id: 'r1', parentId: null, order: 10 }),
      makeNote({ id: 'r2', parentId: null, order: 20 }),
      makeNote({ id: 'r3', parentId: null, order: 30 }),
      makeNote({ id: 'f', parentId: 'r1', isFolder: true, order: 1 }),
      makeNote({ id: 'g', parentId: 'r1', isFolder: true, order: 2 }),
    ];
    const m = buildMoveSupportMap(notes);
    for (const note of notes) {
      const sup = m.get(note.id)!;

      const up = findMoveSibling(notes, note.id, 'up');
      expect(sup.canMoveUp).toBe(up.canMove);
      expect(sup.otherUp?.id).toBe(up.other?.id);

      const down = findMoveSibling(notes, note.id, 'down');
      expect(sup.canMoveDown).toBe(down.canMove);
      expect(sup.otherDown?.id).toBe(down.other?.id);
    }
  });
});
