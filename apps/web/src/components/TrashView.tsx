...
export function TrashView({ onBack }: { onBack: () => void }) {
  ...
  // Empty-Trash confirm backdrop ref + focus trap. The trap captures
  // the Empty Trash trigger as the previously-focused element on mount
  // and restores focus there on close (matching the WAI-ARIA modal
  // dialog expectation that focus returns to the control that opened
  // the dialog).
  const emptyTrashModalRef = useRef<HTMLDivElement | null>(null);
  const { onKeyDown: onEmptyTrashModalKeyDown } = useFocusTrap(
    emptyTrashModalRef,
    showEmptyConfirm,
  );
        ...
        <>
          ...
          {showEmptyConfirm && (
            ...
            >
              ...
              >
                <h3 id="empty-trash-title" className="text-lg font-semibold text-zinc-900 dark:text-zinc-100 mb-2">
                  Empty Trash?
                </h3>
