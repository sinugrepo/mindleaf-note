...
- deploy.sh ssh-based dengan --dry-run + --rollback (dist.bak snapshot sebelum overwrite + auto-rollback pada healthcheck fail). Pre-flight `sudo -n true` check untuk fail-fast (skip under DRY_RUN dengan [dry-run] notice ; tidak berbohong "[ok]").
